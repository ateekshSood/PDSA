...
def neighbors(adjacency_matrix , j , column):

    return [ i for i in range(column) if adjacency_matrix[j , i] == 1]



def bfs(adjacency_matrix , target):
    initial_vertex = 0
    queue = Queue()
    ...
    while(not queue.is_empty()):
        ...
        for k in neighbors(adjacency_matrix , j , column):
